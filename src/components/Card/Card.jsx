    ...
    return (
        <Grid container item dir={'rtl'}>
            ...
            {DataTodos.filter(todo => todo.title.toUpperCase().includes(search.toUpperCase())).map(todo =>
                <Grid key={todo.id}  borderRadius={4} mx={0.5} mb={2} justifyContent={"space-between"} display={"flex"} padding={"16px"} sx={{ background: todo.color }}>
                    <Grid >
                        <Grid item display={"flex"} justifyContent={"center"} alignItems={"center"} >
                            <Grid>
                                <input type={'radio'} checked={todo.state} onClick={() => handelCheck(todo.id)} />
                            </Grid>
                            <Grid mr={2} flexWrap={'wrap'}>
                                <Typography sx={{ textDecoration: todo.state ? 'line-through' : 'unset' }}>{todo.title}</Typography>
